  ...
  <div class="p-6  bg-gray-100 rounded"> 
    ...
    <div class="flex items-center justify-between mb-4">
      <div class="flex items-center space-x-3">
        <button @click="prevWeek" class="px-3 py-1 bg-gray-200  hover:bg-gray-300 rounded">‹</button>
        <div class="text-lg font-semibold">{{ weekRangeLabel }}</div>
        <button @click="nextWeek" class="px-3 py-1 bg-gray-200  hover:bg-gray-300 rounded">›</button>

        <!-- small calendar / pick date to jump -->
        <input type="date" v-model="pickedDate" @change="jumpToDate" class="ml-4 border rounded px-2 py-1" />
      </div>
      ...
      <div class="flex items-center space-x-3">
        ...
        <button @click="openBooking(null)" class=" hover:bg-blue-300 bg-blue-600 text-white px-3 py-1 rounded">New Booking</button>
        <button @click="loadWeek" class="px-3 py-1  bg-white  hover:bg-gray-300 rounded">Refresh</button>
      </div>
    </div>

    <!-- Weekly grid -->
    <div class="bg-white rounded shadow overflow-auto">
      <div class="grid grid-cols-[200px_1fr] border-b bg-gray-50 ${statusClassbg(b.status)}`">
      ...
      <div>
        <div v-for="car in filteredCars" :key="car.id" class="grid grid-cols-[200px_1fr] items-start border-t">
          <!-- car column -->
          <div class="p-3 border-r">
            ...
            <div class="mt-3 space-y-2">
              ...
              <button @click="viewHistory(car)" class="text-sm w-full px-2 py-1 bg-gray-200 hover:bg-gray-100 rounded">ประวัติ-History</button>
            </div>
          </div>
          ...
          <div class="flex">
            <div v-for="d in weekDays" :key="d.date" class="flex-1 p-2 min-h-24 border-l relative">
              <!-- times summary (small) -->
              <div v-if="bookingsByCarDay[String(car.id)] && bookingsByCarDay[String(car.id)][d.date]" class="space-y-2">
                <div v-for="b in bookingsByCarDay[String(car.id)][d.date]" :key="b.id"
                  ...
                  <div class="flex items-start justify-between">
                    <div class="pr-2">
                      ...
                      <div class="text-xxs text-white/90 truncate">{{ b.destination || b.purpose}}</div>
                    </div>

                    <!-- status badge -->
                    <div class="ml-2">
                      <span :class="`inline-block px-2 py-0.5 rounded text-xxs ${statusClass(b.status)}`">
                        {{ b.status }}
                      </span>
                    </div>
                  </div>
                  ...
                  <div class="absolute right-2 top-8 flex space-x-1 ">
                    <button v-if="(isOwner(b) || isAdmin) && String(b?.status || '').toLowerCase() !== 'cancelled'" @click.stop="showCancelConfirm(b)" class="text-xxs px-2 py-0.5 bg-white/20 rounded  hover:bg-white/50 hover:text-white-900 ">Cancel</button>
                    <button v-if="(isOwner(b) || isAdmin) && String(b?.status || '').toLowerCase() !== 'cancelled'" @click.stop="openBookingEdit(b)" class="text-xxs px-2 py-0.5 bg-white/20 rounded  hover:bg-white/50 hover:text-white-900 ">Edit</button>
                  </div>
                </div>
              </div>
      ...
      <div v-if="showBooking" class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="booking-modal-card">
          <div class="booking-modal-header">
            ...
            <div>
              <p class="booking-modal-eyebrow">{{ editingBooking ? 'แก้ไขการจอง' : 'สร้างการจองใหม่' }}</p>
              <h3 class="booking-modal-title">{{ editingBooking ? 'Edit Booking' : 'New Booking' }}</h3>
            </div>
          </div>
          ...
          <form @submit.prevent="submitBooking" class="booking-modal-form">
            ...
            <div class="form-group">
              ...
              <input v-model="form.destination" type="text" placeholder="เช่น สำนักงาน/ลูกค้า/พื้นที่ปฏิบัติงาน" />
            </div>

            <div class="booking-modal-actions">
              <button type="button" @click="closeBooking" class="btn ghost">ยกเลิก</button>
              <button type="submit" class="btn primary">{{ editingBooking ? 'อัปเดต' : 'ยืนยันการจอง' }}</button>
            </div>
          </form>
        </div>
      </div>
        ...
        <div class="bg-white rounded-lg shadow-xl z-50 w-full max-w-md p-6">
          <div class="flex items-start space-x-3">
            <div class="flex-shrink-0">
              <div class="h-10 w-10 rounded-full bg-red-100 flex items-center justify-center text-red-600 font-bold">!</div>
            </div>
            <div class="flex-1">
              ...
              <p class="text-sm text-gray-600 mt-1">
                ...
                (วันที่: {{ smallFormat(cancelTarget?.start_datetime) }} — {{ smallFormat(cancelTarget?.end_datetime) }})
              </p>

              <div class="mt-4 flex justify-end space-x-2">
                <button @click="closeCancel" class="px-3 py-1 bg-gray-100 rounded">ยกเลิก</button>
                <button @click="confirmCancel" class="px-3 py-1 bg-red-600 text-white rounded shadow">ยืนยันยกเลิก</button>
              </div>
            </div>
          </div>
        </div>
...
const weekRangeLabel = computed(() => {
  const a = weekDays.value[0].full, b = weekDays.value[6].full
  return `${a.toLocaleDateString()} — ${b.toLocaleDateString()}`
})
// whenever weekStartDate changes, reload bookings for that week
watch(weekStartDate, () => { loadWeek().catch(()=>{}) })

const prevWeek = () => {
  const d = new Date(weekStartDate.value + 'T00:00:00'); d.setDate(d.getDate()-7); weekStartDate.value = formatDateLocal(d)
}
const nextWeek = () => {
  const d = new Date(weekStartDate.value + 'T00:00:00'); d.setDate(d.getDate()+7); weekStartDate.value = formatDateLocal(d)
}
const jumpToDate = () => {
  const d = new Date(pickedDate.value + 'T00:00:00'); weekStartDate.value = formatDateLocal(startOfWeek(d))
}
...
watch(() => localStorage.getItem('userId'), (v) => customerId.value = Number(v || 0))
console.log('userId=', customerId.value)
// user role / permissions
const userRole = ref(localStorage.getItem('userRole') || localStorage.getItem('role') || '')
watch(() => localStorage.getItem('userRole'), (v) => userRole.value = v || '')
console.log('userRole=', userRole.value)

const isAdmin = computed(() => ['administrator','superadmin'].includes(String(userRole.value).toLowerCase()))
...
function statusClass(status = '') {
  switch (status) {
    ...
    case 'Maintenance': return 'bg-red-200 text-red-900'
    default: return 'bg-gray-100 text-gray-800'
  }
}
function statusClassbg (status = '') {
  switch (status) {
    case 'cancelled': return ' from-red-500 to-red-100'
...
const showPopup = (message: string, options: { title?: string; type?: PopupType; autoCloseMs?: number } = {}) => {
  popup.value = {
    visible: true,
...
const loadWeek = async () => {
  ...
  console.log('[BookingWeekly] loadWeek ->', weekStart.toISOString(), weekEnd.toISOString(), 'filterCar=', filterCar.value)
  ...
  bookings.value.forEach((b: any) => {
    ...
    const lastDay = new Date(to); lastDay.setHours(0,0,0,0)
    while (cur <= lastDay) {
  ...
  Object.values(map).forEach((dayMap: any) => {
    Object.keys(dayMap).forEach((d) => {
      dayMap[d].sort((x:any,y:any) => new Date(x.start_datetime).getTime() - new Date(y.start_datetime).getTime())
    })
  })

  bookingsByCarDay.value = map
  console.log('[BookingWeekly] bookingsByCarDay keys:', Object.keys(bookingsByCarDay.value).slice(0,10))
}
...
const canCancel = (b:any) => b && b.status !== 'cancelled' && b.status !== 'completed' && new Date(b.start_datetime).getTime() >= Date.now() - 5*60000
...
const submitBooking = async () => {
  ...
  if (!form.value.start_date || !form.value.start_time) formErrors.value.push('กรุณาระบุวันและเวลาเริ่มต้น')
  if (!form.value.end_date || !form.value.end_time) formErrors.value.push('กรุณาระบุวันและเวลาสิ้นสุด')
  ...
  const durationDays = Math.ceil((new Date(endIso).getTime() - new Date(startIso).getTime()) / msDay) || 1
  const useDurationRule = false // อนาคตสามารถเปลี่ยนเป็น true เพื่อกลับมาใช้เงื่อนไขเดิม
  const computedStatus = useDurationRule
    ? (durationDays > 2 ? 'pending' : 'approved')
    : 'approved'
  ...
  try {
    const url = form.value.id ? `${API_BASE}/api/booking/${form.value.id}` : `${API_BASE}/api/booking`
    ...
    if (res.status === 409) {
      showPopup(data.message || 'ช่วงเวลานี้มีการจองแล้ว', { type: 'warning', title: 'ไม่สามารถจองได้' })
    } else {
...
const bookingSummaryClass = (b: any) => {
  ...
  const gradient =
    days === 2 ? 'from-indigo-700 to-indigo-400'
    : days > 2 ? 'from-violet-700 to-violet-400'
...
.booking-modal-card > * { position: relative; z-index: 1; }
...
.booking-modal-errors ul { margin-left: 20px; list-style: disc; }
...
.ui-popup-card.type-success { border: 2px solid #22c55e; }
.ui-popup-card.type-error { border: 2px solid #ef4444; }
.ui-popup-card.type-warning { border: 2px solid #f97316; }
.ui-popup-card.type-info { border: 2px solid #3b82f6; }
.ui-popup-title {
  font-weight: 600;
  font-size: 1.05rem;
